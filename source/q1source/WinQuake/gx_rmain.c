...
// double size of eyes, since they are really hard to see while hardware rendering
	...
	/******************* Investigate how to implement these functions ASAP: ***************************************/
...
void R_Mirror (void)
{
	...
	guMtxCopy(r_base_world_matrix, gxu_modelview_matrices[gxu_cur_modelview_matrix]);
	GX_LoadPosMtxImm(gxu_modelview_matrices[gxu_cur_modelview_matrix], GX_PNMTX0);
...
{
	double	time1, time2;
	// These are values required for experimental silly looking fog:
