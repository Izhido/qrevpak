...
// Allocating in big stack. Stack in this device is pretty small:
	...
	byte** rowptr = Sys_BigStackAlloc(sizeof(byte*) * (MAXHEIGHT+(AMP2*2)), "D_WarpScreen");
	int*		column = Sys_BigStackAlloc(sizeof(int) * (MAXWIDTH+(AMP2*2)), "D_WarpScreen");
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree(sizeof(byte*) * (MAXHEIGHT+(AMP2*2))
	                + sizeof(int) * (MAXWIDTH+(AMP2*2)), "D_WarpScreen");
// <<< FIX
}
