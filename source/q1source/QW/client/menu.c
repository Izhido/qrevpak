...
// New variable that tells us if m_state == m_quit (required for the onscreen keyboard handler):
qboolean m_state_is_quit;
// <<< FIX
...
void M_ToggleMenu_f (void)
{
	...
	if (key_dest == key_menu)
	{
		...
		m_state = m_none;
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Resetting variable described above:
		m_state_is_quit = false;
// <<< FIX
		return;
	}
...
/*	case 6:	// music volume
	...
	case 6:	// Wii Remote speed
		wmotespeed.value += dir * 0.5;
		if (wmotespeed.value < 1)
			wmotespeed.value = 1;
		if (wmotespeed.value > 11)
			wmotespeed.value = 11;
		Cvar_SetValue ("wmotespeed", wmotespeed.value);
		break;
	case 7:	// Gamecube controller speed
		gcpadspeed.value += dir * 0.5;
		if (gcpadspeed.value < 1)
			gcpadspeed.value = 1;
		if (gcpadspeed.value > 11)
			gcpadspeed.value = 11;
...
#endif
*/
	M_Print (16, 80, "      Wii Remote Speed");
	r = (wmotespeed.value - 1)/10;
	M_DrawSlider (220, 80, r);

	M_Print (16, 88, "   Gamecube ctl. Speed");
	...
	M_Print (16, 144, "    Use old status bar");
	M_DrawCheckbox (220, 144, cl_sbar.value);

	M_Print (16, 152, "      HUD on left side");
	M_DrawCheckbox (220, 152, cl_hudswap.value);
...
void M_Menu_Quit_f (void)
{
	...
	m_state = m_quit;
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Setting variable described above:
	m_state_is_quit = true;
// <<< FIX
	m_entersound = true;
	msgNumber = rand()&7;
}
...
// Resetting variable described above:
		//m_state = m_main;
		{m_state = m_main;m_state_is_quit = false;}
// <<< FIX
}
...
#if 1
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Bigger text box for additions to credits screen:
