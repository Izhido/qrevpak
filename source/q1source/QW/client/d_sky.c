...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Bug (???) - Visual C++ 2008 detected that snext & tnext is, sometimes, being used without having a valid value
// (it still remains to determine if it's relevant in any way, since it only happens when count == 0 AND spancountminus1 == 0,
// and even then, if s & t are replaced with those unitialized values, they won't be used anymore according to the code above):
				else
				{
					snext = 0;
					tnext = 0;
