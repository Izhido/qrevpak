...
// Allocating in big stack. Stack in this device is pretty small:
	...
	spanpackage_t*	spans = Sys_BigStackAlloc(sizeof(spanpackage_t) * (DPS_MAXSPANS + 1 +
			((CACHE_SIZE - 1) / sizeof(spanpackage_t)) + 1), "D_PolysetDraw");
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree(sizeof(spanpackage_t) * (DPS_MAXSPANS + 1 +
			((CACHE_SIZE - 1) / sizeof(spanpackage_t)) + 1), "D_PolysetDraw");
// <<< FIX
}
