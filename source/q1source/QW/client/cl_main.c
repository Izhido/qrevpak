...
// <<< FIX
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// This constant is not present in the current network library:
...
cvar_t	wmotelookbinv = {"wmotelookbinv","0", true};;
// <<< FIX
...
// Registering new cvar for Wii Remote look button invert:
	Cvar_RegisterVariable (&wmotelookbinv);
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// This would create a "qw" subdirectory in the "current" directory; while valid for the original engine,
// the binary for QWRev is not stored next to baseq2. Since the qw directory would be created anyway
// if any maps were to be downloaded, this is not really needed here:
	//Sys_mkdir("qw");
// <<< FIX
