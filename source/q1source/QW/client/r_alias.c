...
// Allocating in big stack. Stack in this device is pretty small:
	...
	finalvert_t*		finalverts = Sys_BigStackAlloc(sizeof(finalvert_t) * (MAXALIASVERTS +
						((CACHE_SIZE - 1) / sizeof(finalvert_t)) + 1), "R_AliasDrawModel");
	auxvert_t*		auxverts = Sys_BigStackAlloc(sizeof(auxvert_t) * MAXALIASVERTS, "R_AliasDrawModel");
// <<< FIX
...
// Deallocating from previous fixes:
	Sys_BigStackFree(sizeof(finalvert_t) * (MAXALIASVERTS +
						((CACHE_SIZE - 1) / sizeof(finalvert_t)) + 1)
						+ sizeof(auxvert_t) * MAXALIASVERTS, "R_AliasDrawModel");
// <<< FIX
}
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
