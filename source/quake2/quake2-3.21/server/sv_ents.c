...
// Allocating in big stack. Stack in this device is pretty small:
	//int		leafs[64];
	int*		leafs = Sys_BigStackAlloc(64 * sizeof(int), "SV_FatPVS");
// <<< FIX
	int		i, j, count;
	int		longs;
	byte	*src;
...
// Deallocating from previous fix:
	Sys_BigStackFree(64 * sizeof(int), "SV_FatPVS");
// <<< FIX
}
...
// <<< FIX
	...
	buf_data = Sys_BigStackAlloc(32768 * sizeof(byte), "SV_RecordDemoMessage");
	// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(32768 * sizeof(byte), "SV_RecordDemoMessage");
// <<< FIX
}
