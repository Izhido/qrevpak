...
// Allocating in big stack. Stack in this device is pretty small:
	...
	char*		name = Sys_BigStackAlloc(MAX_OSPATH, "SV_BeginDemoserver");
// <<< FIX

// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Adjusting for previous fix:
	//Com_sprintf (name, sizeof(name), "demos/%s", sv.name);
	Com_sprintf (name, MAX_OSPATH, "demos/%s", sv.name);
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree(MAX_OSPATH, "SV_BeginDemoserver");
// <<< FIX
}
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Telling FS_LoadFile to not precache the file:
	//sv_client->downloadsize = FS_LoadFile (name, (void **)&sv_client->download);
	sv_client->downloadsize = FS_LoadFile (name, (void **)&sv_client->download, false);
// <<< FIX
