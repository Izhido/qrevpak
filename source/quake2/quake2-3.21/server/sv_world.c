...
// Allocating for previous fix in big stack:
	leafs = Sys_BigStackAlloc((MAX_TOTAL_ENT_LEAFS) * sizeof(int), "SV_LinkEdict");
	clusters = Sys_BigStackAlloc((MAX_TOTAL_ENT_LEAFS) * sizeof(int), "SV_LinkEdict");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	edict_t		**touch = Sys_BigStackAlloc((MAX_EDICTS) * sizeof(edict_t*), "SV_PointContents");
	edict_t		*hit;
// <<< FIX
	int			i, num;
	int			contents, c2;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree((MAX_EDICTS) * sizeof(edict_t*), "SV_PointContents");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	edict_t		**touchlist = Sys_BigStackAlloc((MAX_EDICTS) * sizeof(edict_t*), "SV_ClipMoveToEntities");
	edict_t *touch;
// <<< FIX
	trace_t		trace;
	int			headnode;
