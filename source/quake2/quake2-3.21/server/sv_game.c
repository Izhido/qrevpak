...
// Allocating in big stack. Stack in this device is pretty small:
	//char		msg[1024];
	char*		msg = Sys_BigStackAlloc(1024, "PF_dprintf");
// <<< FIX
	va_list		argptr;
...
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "PF_dprintf");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char		msg[1024];
	char*		msg = Sys_BigStackAlloc(1024, "PF_cprintf");
// <<< FIX
	va_list		argptr;
	int			n;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "PF_cprintf");
// <<< FIX
}
...
// Allocating for previous fix in big stack:
	msg = Sys_BigStackAlloc(1024, "PF_centerprintf");
// <<< FIX
	va_start (argptr,fmt);
	vsprintf (msg, fmt, argptr);
	va_end (argptr);
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "PF_centerprintf");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char		msg[1024];
	char*		msg = Sys_BigStackAlloc(1024, "PF_error");
// <<< FIX
	va_list		argptr;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "PF_error");
// <<< FIX
}
