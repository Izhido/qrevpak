...
// Allocating for previous fix in big stack:
	string = Sys_BigStackAlloc(1024, "SV_ClientPrintf");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	//char		string[2048];
	char*		string = Sys_BigStackAlloc(2048, "SV_BroadcastPrintf");
// <<< FIX
	client_t	*cl;
	int			i;
...
// Allocating in big stack. Stack in this device is pretty small:
		//char	copy[1024];
		char*	copy = Sys_BigStackAlloc(1024, "SV_BroadcastPrintf");
// <<< FIX
		int		i;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(2048, "SV_BroadcastPrintf");
// <<< FIX
}
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Allocating for previous fix in big stack:
	string = Sys_BigStackAlloc(1024, "SV_BroadcastCommand");
// <<< FIX
	va_start (argptr,fmt);
	vsprintf (string, fmt,argptr);
	va_end (argptr);
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "SV_BroadcastCommand");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	byte*		msg_buf = Sys_BigStackAlloc((MAX_MSGLEN) * sizeof(byte), "SV_SendClientDatagram");
// <<< FIX
	sizebuf_t	msg;
...
// Adjusting for previous fix:
	...
	SZ_Init (&msg, msg_buf, (MAX_MSGLEN) * sizeof(byte));
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree((MAX_MSGLEN) * sizeof(byte), "SV_SendClientDatagram");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	byte*		msgbuf = Sys_BigStackAlloc((MAX_MSGLEN) * sizeof(byte), "SV_SendClientMessages");
// <<< FIX
	int			r;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
				Sys_BigStackFree((MAX_MSGLEN) * sizeof(byte), "SV_SendClientMessages");
// <<< FIX
				return;
			}
