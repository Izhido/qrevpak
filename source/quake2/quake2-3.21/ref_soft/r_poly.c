...
// Allocating in big stack. Stack in this device is pretty small:
	...
	float*	dists = Sys_BigStackAlloc((MAXWORKINGVERTS+3) * sizeof(float), "R_ClipPolyFace");
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree((MAXWORKINGVERTS+3) * sizeof(float), "R_ClipPolyFace");
// <<< FIX
...
// Allocating for previous fix in big stack:
	outverts = Sys_BigStackAlloc((MAXWORKINGVERTS+3) * sizeof(emitpoint_t), "R_ClipAndDrawPoly");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree((MAXWORKINGVERTS+3) * sizeof(emitpoint_t), "R_ClipAndDrawPoly");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	espan_t*	spans = Sys_BigStackAlloc((MAXHEIGHT+1) * sizeof(espan_t), "R_DrawPoly");
// <<< FIX
