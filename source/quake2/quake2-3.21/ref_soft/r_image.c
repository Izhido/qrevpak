...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case out cannot be created:
	if(out == 0)
		Com_Error(ERR_DROP, "LoadPCX: can't allocate %i in out\n", (pcx->ymax+1) * (pcx->xmax+1) );
// <<< FIX
	...
	*pic = out;
	...
	if (palette)
	{
		*palette = malloc(768);
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case palette cannot be created:
		if(palette == 0)
			Com_Error(ERR_DROP, "LoadPCX: can't allocate 768 in palette\n");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case targa_rgba cannot be created:
	if(targa_rgba == 0)
		Com_Error(ERR_DROP, "LoadTGA: can't allocate %i in targa_rgba\n", numPixels*4);
// <<< FIX
	*pic = targa_rgba;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case image->pixels[0] cannot be created:
	if(image->pixels[0] == 0)
		Com_Error(ERR_DROP, "GL_LoadPic: can't allocate %i in image->pixels[0]\n", c);
// <<< FIX
...
// Aborting nicely in case image->pixels[0] cannot be created:
	if(image->pixels[0] == 0)
		Com_Error(ERR_DROP, "R_LoadWal: can't allocate %i in image->pixels[0]\n", size);
// <<< FIX
