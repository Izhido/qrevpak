...
// Aborting nicely in case surfaces cannot be created:
		if(surfaces == 0)
			Com_Error(ERR_DROP, "R_NewMap: can't allocate %i in surfaces\n", r_cnumsurfs * sizeof(surf_t));
// <<< FIX
		surface_p = surfaces;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case auxedges cannot be created:
		if(auxedges == 0)
			Com_Error(ERR_DROP, "R_NewMap: can't allocate %i in auxedges\n", r_numallocatededges * sizeof(edge_t));
// <<< FIX
	}
}
...
// Allocating for previous fix in big stack:
	ledges = Sys_BigStackAlloc((NUMSTACKEDGES +
				((CACHE_SIZE - 1) / sizeof(edge_t)) + 1)
				* sizeof(edge_t), "R_EdgeDrawing");
	lsurfs = Sys_BigStackAlloc((NUMSTACKSURFACES +
				((CACHE_SIZE - 1) / sizeof(surf_t)) + 1)
				* sizeof(surf_t), "R_EdgeDrawing");
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree((NUMSTACKEDGES +
				((CACHE_SIZE - 1) / sizeof(edge_t)) + 1)
				* sizeof(edge_t)
	               + (NUMSTACKSURFACES +
				((CACHE_SIZE - 1) / sizeof(surf_t)) + 1)
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case d_pzbuffer cannot be created:
	if(d_pzbuffer == 0)
		Com_Error(ERR_DROP, "R_InitGraphics: can't allocate %i in d_pzbuffer\n", vid.width*vid.height*2);
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Allocating for previous fix in big stack:
		palette32 = Sys_BigStackAlloc(1024 * sizeof(byte), "R_CinematicSetPalette");
// <<< FIX
		for ( i = 0; i < 256; i++ )
		{
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
		Sys_BigStackFree(1024 * sizeof(byte), "R_CinematicSetPalette");
// <<< FIX
	}
	else
	{
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	char*	pathname = Sys_BigStackAlloc(MAX_QPATH, "R_SetSky");
// <<< FIX
...
// Adjusting for previous fix:
		...
		Com_sprintf (pathname, MAX_QPATH, "env/%s%s.pcx", skyname, suf[r_skysideimage[i]]);
// <<< FIX
		r_skytexinfo[i].image = R_FindImage (pathname, it_sky);
	}
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(MAX_QPATH, "R_SetSky");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char		text[1024];
	char*		text = Sys_BigStackAlloc(1024, "Sys_Error");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "Sys_Error");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char		text[1024];
	char*		text = Sys_BigStackAlloc(1024, "Com_Printf");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "Com_Printf");
// <<< FIX
}
