...
// Precaching the map file if possible:
	//length = FS_LoadFile (name, (void **)&buf);
	length = FS_LoadFile (name, (void **)&buf, true);
// <<< FIX
	if (!buf)
...
// Allocating in big stack. Stack in this device is pretty small:
		//int		leafs[1024];
		int*		leafs = Sys_BigStackAlloc(1024 * sizeof(int), "CM_BoxTrace");
// <<< FIX
		int		i, numleafs;
		vec3_t	c1, c2;
		int		topnode;
...
// Deallocating from previous fix:
		Sys_BigStackFree(1024 * sizeof(int), "CM_BoxTrace");
// <<< FIX
		return trace_trace;
	}
