...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Precaching the script file if possible:
	//len = FS_LoadFile (Cmd_Argv(1), (void **)&f);
	len = FS_LoadFile (Cmd_Argv(1), (void **)&f, true);
// <<< FIX
	if (!f)
	{
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Allocating for previous fix in big stack:
		temporary = Sys_BigStackAlloc(MAX_STRING_CHARS, "Cmd_MacroExpandString");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
		Sys_BigStackFree(MAX_STRING_CHARS, "Cmd_MacroExpandString");
// <<< FIX
		if (++count == 100)
		{
