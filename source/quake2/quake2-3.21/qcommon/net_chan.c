...
// Allocating in big stack. Stack in this device is pretty small:
	...
	byte*		send_buf = Sys_BigStackAlloc((MAX_MSGLEN) * sizeof(byte), "Netchan_OutOfBand");
// <<< FIX
...
// Adjusting for previous fix:
	...
	SZ_Init (&send, send_buf, (MAX_MSGLEN) * sizeof(byte));
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree((MAX_MSGLEN) * sizeof(byte), "Netchan_OutOfBand");
// <<< FIX
}
...
// Allocating for previous fix in big stack:
	send_buf = Sys_BigStackAlloc((MAX_MSGLEN) * sizeof(byte), "Netchan_Transmit");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree((MAX_MSGLEN) * sizeof(byte), "Netchan_Transmit");
// <<< FIX
}
