...
// Precaching the pcx file if possible:
	...
	len = FS_LoadFile (filename, (void **)&raw, true);
// <<< FIX
	if (!raw)
...
// Allocating for previous fix in big stack:
	samples = Sys_BigStackAlloc((22050/14*4) * sizeof(byte), "SCR_ReadNextFrame");
	compressed = Sys_BigStackAlloc(0x20000 * sizeof(byte), "SCR_ReadNextFrame");
// <<< FIX
...
// Deallocating from previous fix:
	Sys_BigStackFree((22050/14*4) * sizeof(byte) + 0x20000 * sizeof(byte), "SCR_ReadNextFrame");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	...
	char*	name = Sys_BigStackAlloc(MAX_OSPATH, "SCR_PlayCinematic");
	char *dot;
// <<< FIX
	int		old_khz;
...
// Adjusting for previous fix:
		...
		Com_sprintf (name, MAX_OSPATH, "pics/%s", arg);
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
		Sys_BigStackFree(MAX_OSPATH, "SCR_PlayCinematic");
// <<< FIX
		return;
	}
...
// Adjusting for previous fix:
	...
	Com_sprintf (name, MAX_OSPATH, "video/%s", arg);
// <<< FIX
