...
// Allocating for previous fix in big stack:
	buffer = Sys_BigStackAlloc(1024, "Con_Dump_f");
	name = Sys_BigStackAlloc(MAX_OSPATH, "Con_Dump_f");
// <<< FIX
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Adjusting for previous fix:
	//Com_sprintf (name, sizeof(name), "%s/%s.txt", FS_Gamedir(), Cmd_Argv(1));
	Com_sprintf (name, MAX_OSPATH, "%s/%s.txt", FS_Gamedir(), Cmd_Argv(1));
// <<< FIX
...
// Deallocating from previous fix:
		Sys_BigStackFree(1024 + MAX_OSPATH, "Con_Dump_f");
// <<< FIX
		return;
	}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char	buffer[1024];
	char*	buffer = Sys_BigStackAlloc(1024, "Con_CenteredPrint");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "Con_CenteredPrint");
// <<< FIX
}
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Allocating for previous fix in big stack:
	version = Sys_BigStackAlloc(64, "Con_DrawConsole");
	dlbar = Sys_BigStackAlloc(1024, "Con_DrawConsole");
// <<< FIX

// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Adjusting for previous fix:
	//Com_sprintf (version, sizeof(version), "v%4.2f", VERSION);
	Com_sprintf (version, 64, "v%4.2f", VERSION);
// <<< FIX
	for (x=0 ; x<5 ; x++)
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(64 + 1024, "Con_DrawConsole");
// <<< FIX
}
