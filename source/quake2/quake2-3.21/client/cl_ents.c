...
// Allocating in big stack. Stack in this device is pretty small:
	...
	char*			model = Sys_BigStackAlloc(MAX_QPATH, "S_RegisterSexedModel");
	char*			buffer = Sys_BigStackAlloc(MAX_QPATH, "S_RegisterSexedModel");
// <<< FIX
...
// Adjusting for previous fix:
	...
	Com_sprintf (buffer, MAX_QPATH, "players/%s/%s", model, base+1);
// <<< FIX
...
// Adjusting for previous fix:
		...
		Com_sprintf (buffer, MAX_QPATH, "players/%s/weapon.md2", model);
// <<< FIX
		mdl = re.RegisterModel(buffer);
		if (!mdl) {
...
// Adjusting for previous fix:
			...
			Com_sprintf (buffer, MAX_QPATH, "players/%s/%s", "male", base+1);
// <<< FIX
			mdl = re.RegisterModel(buffer);
			if (!mdl) {
...
// Adjusting for previous fix:
				...
				Com_sprintf (buffer, MAX_QPATH, "players/male/weapon.md2");
// <<< FIX
				mdl = re.RegisterModel(buffer);
			}
		} 
...
// Deallocating from previous fix:
	Sys_BigStackFree(MAX_QPATH + MAX_QPATH, "S_RegisterSexedModel");
// <<< FIX
