...
// Allocating in big stack. Stack in this device is pretty small:
			//char cmd[1024];
			char* cmd = Sys_BigStackAlloc(1024, "Keys_MenuKey");
// <<< FIX

// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Adjusting for previous fix:
			//Com_sprintf (cmd, sizeof(cmd), "bind \"%s\" \"%s\"\n", Key_KeynumToString(key), bindnames[item->generic.localdata[0]][0]);
			Com_sprintf (cmd, 1024, "bind \"%s\" \"%s\"\n", Key_KeynumToString(key), bindnames[item->generic.localdata[0]][0]);
// <<< FIX
			Cbuf_InsertText (cmd);
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
			Sys_BigStackFree(1024, "Keys_MenuKey");
// <<< FIX
		}
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Placing the controls that set the Wii Remote / Gamecube controller speed:
	s_options_wmotespeed_slider.generic.type	= MTYPE_SLIDER;
	s_options_wmotespeed_slider.generic.x		= 0;
	s_options_wmotespeed_slider.generic.y		= 60;
	s_options_wmotespeed_slider.generic.name	= "Wii Remote speed";
	s_options_wmotespeed_slider.generic.callback = WmoteSpeedFunc;
	s_options_wmotespeed_slider.minvalue		= 2;
	s_options_wmotespeed_slider.maxvalue		= 22;

	s_options_gcpadspeed_slider.generic.type	= MTYPE_SLIDER;
	s_options_gcpadspeed_slider.generic.x		= 0;
	s_options_gcpadspeed_slider.generic.y		= 70;
	s_options_gcpadspeed_slider.generic.name	= "Gamecube ctl. speed";
...
// Adding the new precaching parameter (not really needed here):
	...
	count = FS_LoadFile ("credits", &creditsBuffer, false);
// <<< FIX
	if (count != -1)
	{
		p = creditsBuffer;
...
// Allocating in big stack. Stack in this device is pretty small:
	//char	startmap[1024];
	char*	startmap = Sys_BigStackAlloc(1024, "StartServerActionFunc");
// <<< FIX
	int		timelimit;
	int		fraglimit;
	int		maxclients;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "StartServerActionFunc");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char  mapsname[1024];
	char*  mapsname = Sys_BigStackAlloc(1024, "StartServer_MenuInit");
// <<< FIX
	char *s;
	int length;
	int i;
...
// Adjusting for previous fix:
	...
	Com_sprintf( mapsname, 1024, "%s/maps.lst", FS_Gamedir() );
// <<< FIX
	if ( ( fp = fopen( mapsname, "rb" ) ) == 0 )
	{
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Precaching maps.lst if possible:
		//if ( ( length = FS_LoadFile( "maps.lst", ( void ** ) &buffer ) ) == -1 )
		if ( ( length = FS_LoadFile( "maps.lst", ( void ** ) &buffer, true ) ) == -1 )
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case buffer cannot be created:
		if(buffer == 0)
			Com_Error(ERR_DROP, "StartServer_MenuInit: can't allocate %i in buffer\n", length);
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case mapnames cannot be created:
	if(mapnames == 0)
		Com_Error(ERR_DROP, "StartServer_MenuInit: can't allocate %i in mapnames\n",  sizeof( char * ) * ( nummaps + 1 ) );
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
    ...
    //char  longname[MAX_TOKEN_CHARS];
		//char  scratch[200];
    char*  shortname = Sys_BigStackAlloc(MAX_TOKEN_CHARS, "StartServer_MenuInit");
    char*  longname = Sys_BigStackAlloc(MAX_TOKEN_CHARS, "StartServer_MenuInit");
		char*  scratch = Sys_BigStackAlloc(200, "StartServer_MenuInit");
// <<< FIX
		int		j, l;
...
// Adjusting for previous fix:
		...
		Com_sprintf( scratch, 200, "%s\n%s", longname, shortname );
// <<< FIX

		mapnames[i] = malloc( strlen( scratch ) + 1 );
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case mapnames[i] cannot be created:
		if(mapnames[i] == 0)
			Com_Error(ERR_DROP, "StartServer_MenuInit: can't allocate %i in mapnames[%i]\n", strlen( scratch ) + 1, i);
// <<< FIX
		strcpy( mapnames[i], scratch );
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
    	Sys_BigStackFree(MAX_TOKEN_CHARS + MAX_TOKEN_CHARS + 200, "StartServer_MenuInit");
// <<< FIX
	}
	mapnames[nummaps] = 0;
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024, "StartServer_MenuInit");
// <<< FIX
}
...
// Allocating in big stack. Stack in this device is pretty small:
	//char scratch[1024];
	char* scratch = Sys_BigStackAlloc(1024, "IconOfSkinExists");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
	//char findname[1024];
	//char scratch[1024];
	char* findname = Sys_BigStackAlloc(1024, "PlayerConfig_ScanDirectories");
	char* scratch =  Sys_BigStackAlloc(1024, "PlayerConfig_ScanDirectories");
// <<< FIX
	int ndirs = 0, npms = 0;
	char **dirnames;
	char *path = NULL;
...
// Adjusting for previous fix:
		...
		Com_sprintf( findname, 1024, "%s/players/*.*", path );
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Aborting nicely in case skinnames cannot be created:
		if(skinnames == 0)
			Com_Error(ERR_DROP, "PlayerConfig_ScanDirectories: can't allocate %i in skinnames\n", sizeof( char * ) * ( nskins + 1 ) );
// <<< FIX
...
// Allocating for previous fix in big stack:
	//char currentdirectory[1024];
	//char currentskin[1024];
	currentdirectory = Sys_BigStackAlloc(1024, "PlayerConfig_MenuInit");
	currentskin = Sys_BigStackAlloc(1024, "PlayerConfig_MenuInit");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
	Sys_BigStackFree(1024 + 1024, "PlayerConfig_MenuInit");
// <<< FIX
...
// Allocating in big stack. Stack in this device is pretty small:
		//char scratch[1024];
		char* scratch = Sys_BigStackAlloc(1024, "PlayerConfig_MenuKey");
// <<< FIX
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Deallocating from previous fix:
		Sys_BigStackFree(1024, "PlayerConfig_MenuKey");
// <<< FIX
	}
