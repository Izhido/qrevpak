...
// Allocating in big stack. Stack in this device is pretty small:
		//char buffer[1000];
		char* buffer = Sys_BigStackAlloc(1000, "CL_Setenv_f");
// <<< FIX
		int i;
...
// Deallocating from previous fix:
		Sys_BigStackFree(1000, "CL_Setenv_f");
// <<< FIX
	}
	else if ( argc == 2 )
	{
...
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// Precaching the skin file if possible:
					//FS_LoadFile (cl.configstrings[precache_check], (void **)&precache_model);
					FS_LoadFile (cl.configstrings[precache_check], (void **)&precache_model, true);
// <<< FIX
					if (!precache_model) {
...
// New cvars for speed on the various controllers:
	...
	clsctspeed = Cvar_Get ("clsctspeed", "3", CVAR_ARCHIVE);
// <<< FIX
// >>> FIX: For Nintendo Wii using devkitPPC / libogc
// New cvar for Wii Remote look button invert:
