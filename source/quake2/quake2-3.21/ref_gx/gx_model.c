...
model_t *Mod_ForName (char *name, qboolean crash)
{
	...
	case IDBSPHEADER:
		loadmodel->extradata = Hunk_Begin (0xC00000);//(0x1000000);
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->image = GX_FindImage (name, it_wall);
		if (!out->image)
		{
...
//
	...
	{
		mod->skins[i] = GX_FindImage ((char *)pheader + pheader->ofs_skins + i*MAX_SKINNAME
			, it_skin);
	}
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	for (i=0 ; i<sprout->numframes ; i++)
	{
		...
		mod->skins[i] = GX_FindImage (sprout->frames[i].name,
			it_sprite);
	}
...
struct model_s *R_RegisterModel (char *name)
{
	...
	if (mod)
	{
		...
		if (mod->type == mod_sprite)
		{
			...
			for (i=0 ; i<sprout->numframes ; i++)
				mod->skins[i] = GX_FindImage (sprout->frames[i].name, it_sprite);
		}
		else if (mod->type == mod_alias)
		{
			pheader = (dmdl_t *)mod->extradata;
			for (i=0 ; i<pheader->num_skins ; i++)
				mod->skins[i] = GX_FindImage ((char *)pheader + pheader->ofs_skins + i*MAX_SKINNAME, it_skin);
//PGM
